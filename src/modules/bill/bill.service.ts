...
const calculateBillAmount = async (units: number) => {
	if (units < 0) {
		throw new AppError(400, "Units cannot be negative");
	}
	...
	if (tariffs.length === 0) {
		throw new AppError(404, "No active tariff found");
	}
	...
	if (remainingUnits > 0) {
		throw new AppError(400, "No tariff found for all consumed units");
	}
...
const createBill = async (payload: ICreateBill) => {
	...
	if (!user.email) {
		throw new AppError(400, "Customer email is not available");
	}
