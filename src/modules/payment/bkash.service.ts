	...
	const response = await fetch(
		`${config.bkash_base_url}/tokenized/checkout/create`,
		{
			...
			body: JSON.stringify({
				...
				merchantInvoiceNumber: `INV-${bill.id}`,
			}),
		},
	);
...
export const executeBkashPayment = async (paymentID: string) => {
	...
	const invoicePdf = await pdfService.generatePaymentInvoicePdf({
		invoiceNumber,

		customerName: paymentDetails.user.name,

		customerEmail: paymentDetails.user.email,

		meterNumber: paymentDetails.bill.meterNumber,

		billId: paymentDetails.bill.id,
		...
		paymentDate: paymentDetails.updatedAt,
